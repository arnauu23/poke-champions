        ...
        if (selectedPokemon) {
            fetch(selectedPokemon.url)
                ...
                .then(async data => {
                    ...
                    // CÁLCULO DE DEBILIDADES MATEMÁTICAS
                    try {
                        const typeUrls = data.types.map(t => t.type.url);
                        const typeResponses = await Promise.all(typeUrls.map(url => fetch(url).then(r => r.json())));

                        let multis = { normal: 1, fire: 1, water: 1, electric: 1, grass: 1, ice: 1, fighting: 1, poison: 1, ground: 1, flying: 1, psychic: 1, bug: 1, rock: 1, ghost: 1, dragon: 1, dark: 1, steel: 1, fairy: 1 };

                        typeResponses.forEach(typeData => {
                            const relations = typeData.damage_relations;
                            relations.double_damage_from?.forEach(t => multis[t.name] *= 2);
                            relations.half_damage_from?.forEach(t => multis[t.name] *= 0.5);
                            relations.no_damage_from?.forEach(t => multis[t.name] *= 0);
                        });

                        setWeaknesses(multis);
                    } catch (err) {
                        console.error("Error calculando debilidades:", err);
                        setWeaknesses(null);
                    }
                        ...
                        {Object.keys(TYPE_DATA).map(type => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold uppercase transition-all shadow-sm ${isSelected ? `${TYPE_DATA[type].bg} text-white ring-2 ring-offset-2 ring-offset-white dark:ring-offset-slate-800 ring-slate-400` : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600'
                                        }`}
                                >
                        ...
                        {GENS.map(gen => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`px-3 py-2 rounded-lg text-sm font-bold transition-all shadow-sm border ${isSelected ? 'bg-red-500 text-white border-red-600 ring-2 ring-offset-2 ring-offset-white dark:ring-offset-slate-800 ring-red-300' : 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-600 hover:bg-slate-200 dark:hover:bg-slate-600'
                                        }`}
                                >
                                    {gen.id}
                                </button>
                ...
                <div className="flex flex-col xl:flex-row gap-6 bg-slate-50 dark:bg-slate-900 p-6 rounded-xl border-2 border-slate-200 dark:border-slate-700 transition-colors">
                    ...
                    {weaknesses && (
                        <div className="w-full mt-6 bg-slate-100 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                            <h3 className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-3 text-center uppercase tracking-widest">Efectividad de Tipos</h3>

                            <div className="flex flex-wrap justify-center gap-3">
                                {[
                                    { mult: 4, label: "x4", desc: "Súper Débil", color: "text-red-600 dark:text-red-400", bg: "bg-red-50 dark:bg-red-900/20", border: "border-red-200 dark:border-red-800" },
                                    { mult: 2, label: "x2", desc: "Débil", color: "text-orange-600 dark:text-orange-400", bg: "bg-orange-50 dark:bg-orange-900/20", border: "border-orange-200 dark:border-orange-800" },
                                    { mult: 0.5, label: "x0.5", desc: "Resistente", color: "text-blue-600 dark:text-blue-400", bg: "bg-blue-50 dark:bg-blue-900/20", border: "border-blue-200 dark:border-blue-800" },
                                    { mult: 0.25, label: "x0.25", desc: "Muy Resist.", color: "text-green-600 dark:text-green-400", bg: "bg-green-50 dark:bg-green-900/20", border: "border-green-200 dark:border-green-800" },
                                    { mult: 0, label: "x0", desc: "Inmune", color: "text-slate-600 dark:text-slate-400", bg: "bg-slate-200 dark:bg-slate-700", border: "border-slate-300 dark:border-slate-600" }
                                ].map(group => {
                                    const typesInGroup = Object.entries(weaknesses).filter(([_, val]) => val === group.mult).map(([type]) => type);

                                    if (typesInGroup.length === 0) return null; // Oculta la tarjeta si no tiene tipos

                                    return (
                                        <div key={group.mult} className={`flex flex-col items-center p-2.5 rounded-xl border ${group.bg} ${group.border} min-w-[90px] shadow-sm`}>
                                            <span className={`text-base font-black leading-none ${group.color}`}>{group.label}</span>
                                            <span className={`text-[9px] uppercase tracking-wider font-bold mt-1 mb-2 ${group.color} opacity-80`}>{group.desc}</span>
                                            <div className="flex flex-wrap justify-center gap-1.5">
                                                {typesInGroup.map(type => {
                                                    const typeData = TYPE_DATA[type];
                                                    return (
                                                        <span key={type} className={`flex items-center gap-1 px-1.5 py-0.5 ${typeData?.bg || 'bg-slate-500'} text-white rounded-md text-[10px] uppercase font-bold shadow-sm`} title={type}>
                                                            <img src={typeData?.icon} alt={type} className="w-3.5 h-3.5 drop-shadow-sm" />
                                                            {type}
                                                        </span>
                                                    );
                                                })}
